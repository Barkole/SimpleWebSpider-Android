...
/**
 * A array-based <tt>List</tt> implementation with <em>Soft keys</em>.
 * An entry in a <tt>SoftArrayList</tt> will automatically be removed when
 * it is no longer in ordinary use.
 *
 * <p>The behavior of the <tt>SoftArrayList</tt> class depends in part upon
 ...
 * <tt>SoftArrayList</tt> may behave as though an unknown thread is silently
 * removing entries.  In particular, even if you synchronize on a
 * <tt>SoftArrayList</tt> instance and invoke none of its mutator methods, it
...
public class SoftArrayList<T> extends AbstractList<T> {
	 ...
	 */
	public SoftArrayList(Collection<? extends T> collection) {
	 ...
	 *
	 * @param obj is the object on which a Soft reference must be attached.
	 * @return the Soft reference.
	 */
	private Reference<T> createRef(T obj) {
		return new SoftReference<>(maskNull(obj), this.queue);
	}

	/**
	 * Increases the capacity of this <tt>SoftArrayList</tt> instance, if
	...
	/**
	 * Trims the capacity of this <tt>SoftArrayList</tt> instance to be the
	 * list's current size.  An application can use this operation to minimize
	 * the storage of an <tt>SoftArrayList</tt> instance.
	 */
	...
	protected void assertRange(int index, boolean allowLast) {
		...
		if (index < 0) {
			throw new IndexOutOfBoundsException(String.format("Invalid negative value for index: %s", index)); //$NON-NLS-1$
		}
		if (allowLast && (index > csize)) {
			throw new IndexOutOfBoundsException(String.format("Index must be greater than %s. Current value is: %s", csize, index)); //$NON-NLS-1$
		}
		if (!allowLast && (index >= csize)) {
			throw new IndexOutOfBoundsException(String.format("Index must be greater than or equal to %s. Current value is: %s", csize, index)); //$NON-NLS-1$
		}
	}
